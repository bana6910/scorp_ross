...
train <- fread("C:/Users/jcotrell/Documents/Project 1/scorp_ross/input_data/train.csv") #Note strange error obtained
...
#Convert date field to type date
  ...
  #these lines didn't appear to work; resulted in all NA's
    train[, Date := as.Date(Date,"%m/%d/%Y")] #what does this do?
    #test[, Date := as.Date(Date)]   #what does this do?
...
train_store$Sales = as.numeric(train_store$Sales) #necessary for functions like sum()

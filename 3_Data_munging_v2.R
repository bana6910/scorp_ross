...
# Creating column for the year and week
    # Add columns to f_train_store and f_test_store for year and week number
    f_train_store$Year = as.numeric(year(f_train_store$Date))
    f_test_store$Year = as.numeric(year(f_test_store$Date))
    
    f_train_store$Week = as.numeric(week(f_train_store$Date))
    f_test_store$Week = as.numeric(week(f_test_store$Date))

 # # Creating factor for the week type
 #    f_train_store.n0 = f_train_store[f_train_store$Sales != 0,] #remove zeros for plotting
    ggplot(f_train_store.n0, aes(factor(week(Date)), Sales)) + geom_boxplot() #weeks 18, 31, 45, 48 and 49, and 51 are exceptionally good; weeks 3,7,11,26, 28, 30, 38, 42, 46, 53 are poor
 #    
    # f_train_store$GoodWeek = 0
    # f_train_store$GoodWeek[f_train_store$Week %in% c(18,31,45,48,49,51)] = 1
    # f_train_store$GoodWeek[f_train_store$Week %in% c(3,7,11,26,28,30,38,42,46,53)] = -1
    # f_train_store$GoodWeek = as.factor(f_train_store$GoodWeek)
    # summary(f_train_store$GoodWeek)
    # 
    # #repeat calculation for test data
    # f_test_store$GoodWeek = 0
    # f_test_store$GoodWeek[f_test_store$Week %in% c(18,31,45,48,49,51)] = 1
    # f_test_store$GoodWeek[f_test_store$Week %in% c(3,7,11,26,28,30,38,42,46,53)] = -1
    # f_test_store$GoodWeek = as.factor(f_test_store$GoodWeek)
    # summary(f_test_store$GoodWeek)

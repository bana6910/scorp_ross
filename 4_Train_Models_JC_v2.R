...
# Creating six level factor for prediction weeks [for hyperparameter Tuning Only; modify weeks for final run]
    
    # #Identify Weeks of interest (6 week period) #weeks 19 to 25
    # InterestWeekMax = week(max(f_train_store$Date))
    # InterestWeekMin = InterestWeekMax-6
    # 
    # f_train_store$PredictionWeek = 0
    # f_train_store$PredictionWeek[f_train_store$Week==19] = 19
    # f_train_store$PredictionWeek[f_train_store$Week==20] = 20
    # f_train_store$PredictionWeek[f_train_store$Week==21] = 21
    # f_train_store$PredictionWeek[f_train_store$Week==22] = 22
    # f_train_store$PredictionWeek[f_train_store$Week==23] = 23
    # f_train_store$PredictionWeek[f_train_store$Week==24] = 24
    # f_train_store$PredictionWeek[f_train_store$Week==25] = 25
    # f_train_store$PredictionWeek = as.factor(f_train_store$PredictionWeek)
    # str(f_train_store$PredictionWeek)
    # summary(f_train_store$PredictionWeek)
    
    #repeat calculation for test data
      #Doesn't need to be done here because the test data is derived from this data. But does need to be added to the prediction script

#Remove the last 6 weeks from f_train_store for timeseries training and testing
 train_d = f_train_store[Date < max(f_train_store$Date - 6*7),]
 test_d = f_train_store[Date >= max(f_train_store$Date - 6*7),]
 

 # f_train_store$GoodWeek[f_train_store$Week %in% c(18,31,45,48,49,51)] = 1
 train_d = f_train_store[Date < max(f_train_store$Date - 6*7),]
 test_d = f_train_store[Date >= max(f_train_store$Date - 6*7),]


 #Remove all training data that has outliers from the Holidays
 ggplot(f_train_store.n0, aes(factor(week(Date)), Sales)) + geom_boxplot() 
   
   #Remove unusual months
   train_d = f_train_store[f_train_store$Week %in% c(1:48)]
  
  # # #Sample from train_d to reduce the amount of training data processed for computational efficiency while selecting hyper parameters
    # set.seed(123) #set a seed to provide some repeatability when sampling
    # 
    # fraction = .02
    # train_ind = sample(nrow(train_d), nrow(train_d)*fraction) #creates a training index vector of size nrow*fraction.
    # train_d = train_d[train_ind,]#create the training and test data sets by pulling all indexed rows from d
 ...
 #         
    ...
    save(gbm.model, file = "model_gbmf_train_store.n0 = f_train_store[f_train_store$Sales != 0,].RData")
    print(gbm.model) 
    #plot(gbm.model) 
    

    #Comput RSME for log of sales value (the estimate for RMSPE)
    #The final values used for the model were n.trees = 1000, interaction.depth = 20, shrinkage = 0.1 and n.minobsinnode = 3.
    gbm.pred = predict(gbm.model, newdata=test_d.n0)
    rmse.gbm.pred = rmse(test_d.n0$Sales, gbm.pred);rmse.gbm.pred 
    
    #Check that RMSPE is being computed correctly
    # RMSPE.cb6 =  sqrt( (sum( (test_d.n0$Sales - cb.pred6)/test_d.n0$Sales )^2 ) / nrow(test_d.n0) ); RMSPE.cb6 #56%
    actual = exp(test_d.n0$Sales)
    predicted =exp(gbm.pred)
    RMSPE.gbm = rmspe(actual,predicted); RMSPE.gbm 

...
#script to condition (merge and clean) the training data and test data should have been run previously


  #Set Working Directory, Read Data, and Check Training Data
    setwd("C:/Users/jcotrell/OneDrive/BANA MS/6910 Practicum/Project 1")
    data.train <- read.csv("train.merged.stores.csv")
    data.test <- read.csv("train.merged.stores.csv")
  ...
  #Convert variables to dates and factors as needed
    data.train$Store = as.factor(data$Store)
    #data$Store = as.integer(data$Store) #Changed back to integer due to too many levels
    data.train$DayOfWeek = as.factor(data$DayOfWeek)
    data.train$Date = as.Date(data$Date,"%m/%d/%Y")
    data.train$Open = as.factor(data$Open)
    data.train$Promo = as.factor(data$Promo)
    data.train$StateHoliday = as.factor(data$StateHoliday)
    data.train$SchoolHoliday = as.factor(data$SchoolHoliday)
    data.train$StoreType = as.factor(data$StoreType)
    data.train$Assortment = as.factor(data$Assortment)
    data.train$CompetitionOpenSinceYear = as.factor(data$CompetitionOpenSinceYear)
    data.train$Promo2 = as.factor(data$Promo2)
    data.train$Promo2SinceYear = as.factor(data$Promo2SinceYear)
